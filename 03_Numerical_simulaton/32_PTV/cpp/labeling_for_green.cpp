...
/* 進捗表示 */
const char program_name[] = "LABELING FOR GREEN IMAGE";
...
int main()
{
    ...
    /* ディレクトリの作成 */
    char dirname[2][100];
    sprintf(dirname[0], "%s/%s/LLS_2/labeling_position_dat", dir_path, name);
    sprintf(dirname[1], "%s/%s/LLS_2/labeling_image_png", dir_path, name);
    ...
    for (int m = 1; m <= data_num; m++)
    {
        // 粒子画像の読み込み
        char readfile[100];
        sprintf(readfile, "%s/%s/LLS_2/particle_image_bmp/%d.bmp", dir_path, name, m);
        ...
        // 各座標ごとに書き出し
        char writefile[100];
        sprintf(writefile, "%s/%s/LLS_2/labeling_position_dat/%d.dat", dir_path, name, m);
        ...
        if (m <= 100)
        {
            ...
            sprintf(graphname, "%s/%s/LLS_2/labeling_image_png/%d.png", dir_path, name, m);
            sprintf(backimage, "%s/%s/LLS_2/particle_image_png/%d.png", dir_path, name, m);
