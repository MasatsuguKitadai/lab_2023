...
/** プロトタイプ宣言 **/
void Velocity(const char *name, const char *data_set, int n);
void Vorticity(const char *name, const char *data_set, int n);
void Plot_Velocity(const char *name, const char *data_set, int n);
...
int main()
{
    ...
    for (int n = 0; n < number / (frame_rate * sec); n++)
    {
        printf("n = %d\n", n);
        Velocity(name, data_set, n);
        Plot_Velocity(name, data_set, n);
    }
...
void Velocity(const char *name, const char *data_set, int n)
{
    ...
    /* ディレクトリの作成 */
    char dirname[100];
    sprintf(dirname, "%s/%s/43_PTV/%s/PTV_velocity_dat", dir_path, name, data_set);
    ...
    char writefile[100];
    sprintf(writefile, "%s/%s/43_PTV/%s/PTV_velocity_dat/velocity_%d.dat", dir_path, name, data_set, n);
...
void Plot_Velocity(const char *name, const char *data_set, int n)
{
    /* ディレクトリの作成 */
    char dirname[100];
    sprintf(dirname, "%s/%s/43_PTV/%s/PTV_velocity_svg", dir_path, name, data_set);
    mkdir(dirname, dirmode);
    ...
    sprintf(graphfile, "%s/%s/43_PTV/%s/PTV_velocity_svg/velocity_%d.svg", dir_path, name, data_set, n);
    ...
    fprintf(gp, "set cblabel offset 1.0, 0.0\n");
